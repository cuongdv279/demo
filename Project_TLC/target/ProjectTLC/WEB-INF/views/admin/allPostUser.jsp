                    ...
                    <div class="panel-body">
                        <div class="dataTable_wrapper">
                            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                <c:forEach var="allTopicUser" items="${allTopicUser }" varStatus="status">
                                    <tr class="odd gradeX">
                                        <td>${status.index + 1 }</td>
                                        <td>${allTopicUser.post_ID}</td>
                                        <td><a href="editPostUser.html">${allTopicUser.post_Name}</a></td>
                                        <td><a href="<c:url value="/admin/profile/user/${allTopicUser.user_ID.user_ID}"/>">${allTopicUser.user_ID.user_ID}</a></td>
                                        <c:if test="${allTopicUser.status == 0}">
                                            <td>Draft</td>
                                        </c:if>
                                        <c:if test="${allTopicUser.status == 1}">
                                            <td>Public</td>
                                        </c:if>
                                        <c:if test="${allTopicUser.status == 3}">
                                            <td>Bin</td>
                                        </c:if>
                                        <td>${allTopicUser.last_Edit}</td>
                                        <td>
                                            <a class="btn" href="<c:url value="/admin/all/topicuser/${allTopicUser.post_ID}"/> " >Edit</a>
                                            <span>|</span>
                                            <a class="btn" href="<c:url value="/admin/all/topicuser/${allTopicUser.post_ID}/delete"/>" >Delete</a>
                                        </td>
                                    </tr>
                                </c:forEach>
                                </tbody>
