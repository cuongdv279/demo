...
%>
    <div id="page-wrapper">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header">List User's Post</h1>
            </div>
            <!-- /.col-lg-12 -->
        </div>
        <!-- /.row -->
                    ...
                    <div class="panel-body">
                        <div class="dataTable_wrapper">
                            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                <c:forEach var="allTopicPublicUser" items="${allTopicPublicUser }" varStatus="status">
                                    <tr class="odd gradeX">
                                        <td>${status.index + 1 }</td>
                                        <td>${allTopicPublicUser.post_ID}</td>
                                        <td><a href="editPostUser.html">${allTopicPublicUser.post_Name}</a></td>
                                        <td><a href="<c:url value="/admin/profile/user/${allTopicPublicUser.user_ID.user_ID}"/>">${allTopicPublicUser.user_ID.user_ID}</a></td>
                                        <c:if test="${allTopicPublicUser.status == 0}">
                                            <td>Draft</td>
                                        </c:if>
                                        <c:if test="${allTopicPublicUser.status == 1}">
                                            <td>Public</td>
                                        </c:if>
                                        <c:if test="${allTopicPublicUser.status == 3}">
                                            <td>Bin</td>
                                        </c:if>
                                        <td>${allTopicPublicUser.last_Edit}</td>
                                        <td>
                                            <a class="btn" href="<c:url value="/admin/publicTopic/topicuser/${allTopicPublicUser.post_ID}"/> " >Edit</a>
                                            <span>|</span>
                                            <a class="btn" href="<c:url value="/admin/publicTopic/topicuser/${allTopicPublicUser.post_ID}/delete"/>" >Delete</a>
                                        </td>
                                    </tr>
                                </c:forEach>
                                </tbody>
