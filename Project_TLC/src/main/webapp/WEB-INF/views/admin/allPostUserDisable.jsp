                    ...
                    <div class="panel-body">
                        <div class="dataTable_wrapper">
                            <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                <c:forEach var="allTopicBinUser" items="${allTopicBinUser }" varStatus="status">
                                    <tr class="odd gradeX">
                                        <td>${status.index + 1 }</td>
                                        <td>${allTopicBinUser.post_ID}</td>
                                        <td><a href="editPostUser.html">${allTopicBinUser.post_Name}</a></td>
                                        <td><a href="<c:url value="/admin/profile/user/${allTopicBinUser.user_ID.user_ID}"/>">${allTopicBinUser.user_ID.user_ID}</a></td>
                                        <c:if test="${allTopicBinUser.status == 0}">
                                            <td>Draft</td>
                                        </c:if>
                                        <c:if test="${allTopicBinUser.status == 1}">
                                            <td>Public</td>
                                        </c:if>
                                        <c:if test="${allTopicBinUser.status == 3}">
                                            <td>Bin</td>
                                        </c:if>
                                        <td>${allTopicBinUser.last_Edit}</td>
                                        <td>
                                            <a class="btn" href="<c:url value="/admin/binTopic/topicuser/${allTopicBinUser.post_ID}"/> " >Edit</a>
                                            <span>|</span>
                                            <a class="btn" href="<c:url value="/admin/binTopic/topicuser/${allTopicBinUser.post_ID}/delete"/>" >Delete</a>
                                        </td>
                                    </tr>
                                </c:forEach>
                                </tbody>
